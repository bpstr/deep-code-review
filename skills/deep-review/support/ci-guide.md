...
Architecture reviews use the same completion and confidence contracts. Confirmed maintainability debt can retain high confidence while remaining P2. Optional scanner limitations and unverified imported-report freshness appear in both Markdown and the JSON `coverage_notes` array. They describe additional scanner coverage; a failed required reviewer or shared profile still makes the review incomplete. For architectural findings without baseline evidence, extraction records `PRE-EXISTING` with introduction time unknown in the details so they cannot become invented NEW gate failures.
