...
# A live occupied slot must not make a worker wait forever. The shim should expose a
# queued lifecycle state and fail with a retryable slot-timeout exit.
printf '%s\n%s\n' "$$" "$boot_id" >"$TMP/slots/1"
set +e
...
# Cancelling a shim must keep ownership until its provider has really terminated. A
# provider that ignores TERM is escalated to KILL after the configured grace period.
DEEP_REVIEW_REAL_CODEX="$TMP/bin/fake-codex" \
DEEP_REVIEW_PERSISTENT_RUN_DIR="$TMP/run" \
DEEP_REVIEW_ACTIVE_WORK_DIR="$TMP/work" \
DEEP_REVIEW_GLOBAL_SLOT_DIR="$TMP/slots" \
...
# A running provider also has a bounded execution time; timeout follows the same
# terminate/escalate path and leaves no slot or child behind.
rm -f "$TMP/provider.started" "$TMP/provider.pid"
rm -rf "$TMP/work/lifecycle"
DEEP_REVIEW_REAL_CODEX="$TMP/bin/fake-codex" \
DEEP_REVIEW_PERSISTENT_RUN_DIR="$TMP/run" \
DEEP_REVIEW_ACTIVE_WORK_DIR="$TMP/work" \
...
echo "provider lifecycle regression test passed"
